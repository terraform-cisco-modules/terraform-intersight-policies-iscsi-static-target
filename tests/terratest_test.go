...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-iscsi-s-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} iSCSI Static Target Policy.",

	"IpAddress": "198.18.32.60",
	"Lun": {
	  "Bootable": true,
	  "ClassId": "vnic.Lun",
	  "LunId": 0,
	...
	"Port": 3260,
	"TargetName": "iqn.1992-08.com.ci:sn.e461ee9f190611ebb06300a0985b4a87:vs.3"
}
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/IscsiStaticTargetPolicies/%s", moid), expectedJSONTemplate, vars)
}
